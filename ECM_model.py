...
class ECMModel(object):
    ...
    def setup_system(self):
        ...
        if not self.forward_only:
            ...
            self.real_learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step, 10000, 0.99, staircase=True)
            self.train_op = tf.train.AdamOptimizer(self.real_learning_rate, beta1=0.5).minimize(self.tfloss, global_step=global_step)
        else:
    ...
    def train(self, sess, training_set, tensorboard=False):
        ...
        if tensorboard:
            ...
            _, loss, merged, lr = sess.run([self.train_op, self.tfloss, self.merged_all, self.real_learning_rate], feed_dict=input_feed, options=run_options, run_metadata=run_metadata)
